...
def save_snapshot(r_window, t_window):
    data = np.column_stack((lf_rb_x.get_x_buffers(NUM_FRAMES), lf_rb_y.get_x_buffers(NUM_FRAMES), r_window, t_window))
...
print(list(device.demods[1].child_nodes(recursive=True, leavesonly=True)))
...
device.demods[1].sample.unsubscribe()
...
#lf_ch = Channel(0, NUM_SUB_BUFFERS)
#hf_ch = Channel(1, NUM_SUB_BUFFERs)

lf_rb_x = rolling_buffer(buffer_length = NUM_SUB_BUFFERS)
lf_rb_y = rolling_buffer(buffer_length = NUM_SUB_BUFFERS)
lf_rb_r = rolling_buffer(buffer_length = NUM_SUB_BUFFERS)
lf_rb_phase = rolling_buffer(buffer_length = NUM_SUB_BUFFERS)

hf_rb_x = rolling_buffer(buffer_length = NUM_SUB_BUFFERS)
hf_rb_y = rolling_buffer(buffer_length = NUM_SUB_BUFFERS)
hf_rb_r = rolling_buffer(buffer_length = NUM_SUB_BUFFERS)
...
for i in range(NUM_LOOPS):
    ...
    if i == 0:
        print("poll_result keys:")
        for k in poll_result.keys():
            print(k)
    ...
    try:
        ...
        lf_x = poll_result[device.demods[0].sample]["x"]
        hf_x = poll_result[device.demods[1].sample]["x"]
        ...
        lf_rb_r.add_sub_buffer(lf_r)
        lf_rb_phase.add_sub_buffer(lf_phase)
    ...
    if is_threshold_breached:
        ...
        if (peak_voltage_dif > MAX_VOLTAGE_THRESHOLD and peak_time_dif > 0 and ((t_window[min_index] > prev_timestamps[0]) and (t_window[max_index] > prev_timestamps[0])) and prev_peak_voltage_dif >= peak_voltage_dif):
            ...
            print(f"low-frequency phase: {lf_phase_at_peak}\n" \
                  ...
                  f"high-frequency centered phase: {hf_phase_centered}\n" \
                  f"Window? - LF: {is_lf_in_window} HF: {is_hf_in_window}") 
