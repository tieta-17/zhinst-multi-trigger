...
def handle_commands(line):
	global MAX_VOLTAGE_THRESHOLD, POLARITY_FLIPPED
	...
	if line.lower().startswith("t"):
		try:
			_, val = line.split()
			MAX_VOLTAGE_THRESHOLD = float(val)
...
def calculate_delay_and_trigger(peak_time_dif, pin, peak_timestamp):
	...
	trigger_delay = 0.07 - peak_current_time_dif - INST_SAMPLE_DELAY #TRIGGER_DELAY_SCALE*peak_time_dif - peak_current_time_dif
...
lf_samples = device.demods[lf_ch.demod_idx].sample
hf_samples = device.demods[hf_ch.demod_idx].sample
...
for i in range(NUM_LOOPS):
	...
	try:
		...
		lf_data = poll_result[lf_samples]
		hf_data = poll_result[hf_samples]
	...
	if is_threshold_breached:
		...
		if (peak_voltage_dif > MAX_VOLTAGE_THRESHOLD and peak_time_dif > 0 and ((t_window[min_index] > prev_timestamps[0]) and (t_window[max_index] > prev_timestamps[0])) and prev_peak_voltage_dif >= peak_voltage_dif):
			...
			if (is_lf_in_window and is_hf_in_window):
				...
				# Peak Statistics
				print(f"Vmax  = {lf_r_window[max_index]:.3f} V |") 
				print(f"Vmin  = {lf_r_window[min_index]:.3f} V |") 
